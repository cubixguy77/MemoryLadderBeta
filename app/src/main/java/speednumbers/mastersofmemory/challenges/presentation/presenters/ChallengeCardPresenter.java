package speednumbers.mastersofmemory.challenges.presentation.presenters;

import speednumbers.mastersofmemory.challenges.domain.model.TimeModel;
import speednumbers.mastersofmemory.challenges.presentation.IChallengeCard;

public class ChallengeCardPresenter implements IChallengeCard.Presenter {
    public ChallengeCardPresenter() {

    }

    @Override
    public void onPlayClick() {

    }

    @Override
    public void onExpand() {

    }

    @Override
    public void onContract() {

    }

    @Override
    public void onMemTimerEnable() {

    }

    @Override
    public void onMemTimerDisable() {

    }

    @Override
    public void onMemTimeClick() {

    }

    @Override
    public void onMemTimeUpdated(TimeModel newTimeModel) {

    }

    @Override
    public void onRecallTimerEnable() {

    }

    @Override
    public void onRecallTimerDisable() {

    }

    @Override
    public void onRecallTimeClick() {

    }

    @Override
    public void onRecallTimeUpdated(TimeModel newTimeModel) {

    }
}
